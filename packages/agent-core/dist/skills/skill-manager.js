...
export class InMemorySkillRegistry {
    ...
    async match(intent, domain, entityTypes) {
        const matches = [];
        ...
        for (const skill of this.skills.values()) {
            ...
            if (entityTypes && trigger.entityTypes.length > 0) {
                const overlap = entityTypes.filter((e) => trigger.entityTypes.some((t) => t.toLowerCase() === e.toLowerCase()));
                score += overlap.length;
            }
            // Maturity bonus
            if (skill.maturity === "mature")
                score += 2;
            else if (skill.maturity === "committed")
                score += 1;
            if (score > 0) {
    ...
    async recordOutcome(skillId, success) {
        ...
        if (newMaturity !== skill.maturity) {
            this.log.info({ id: skillId, name: skill.name, from: skill.maturity, to: newMaturity }, "skill maturity advanced");
        }
    }
    advanceMaturity(current, count, rate) {
        if (rate < ADVANCE_MIN_SUCCESS)
            return current;
        if (current === "progenitor" && count >= COMMITTED_THRESHOLD)
            return "committed";
        if (current === "committed" && count >= MATURE_THRESHOLD)
            return "mature";
        return current;
    }
}
...
export class SkillManager {
    ...
    memory;
    log;
    constructor(registry, memory, logger) {
    ...
    async removePlugin(skillId) {
        await this.registry.deregister(skillId);
    }
    /** Remove a skill by name. */
    async removePluginByName(name) {
     ...
     */
    async matchSkills(perception) {
        const entityTypes = perception.entities.map((e) => e.type);
        return this.registry.match(perception.intent, perception.domain, entityTypes);
    }
    ...
    async tryCrystallize() {
        ...
        try {
            ...
            if (successful.length < CRYSTALLIZATION_THRESHOLD)
                return newSkills;
            ...
            for (const pattern of patterns) {
                ...
                this.log.info({ name: skill.name, intentPatterns: pattern.intentPatterns }, "skill crystallized from episode patterns");
            }
        }
        catch (err) {
    ...
    detectPatterns(episodes) {
        ...
        for (const [actionKey, group] of byAction) {
            if (group.length < CRYSTALLIZATION_THRESHOLD)
                continue;
