...
export class PlanningEngine {
     ...
     */
    async createPlan(reasoning, availableTools, behavior) {
        ...
        if (this.llmClient) {
            ...
            if (llmSteps) {
                ...
                this.log.debug({ stepCount: llmSteps.length, parallelGroups: parallelGroups.length }, "LLM plan created");
                return plan;
            }
        }
        ...
        this.log.debug({ stepCount: steps.length, parallelGroups: parallelGroups.length }, "Plan created");
        return plan;
    }
    /**
     ...
     */
    async replan(plan, failedStepId, error) {
        ...
        if (failedStep?.fallbackAction) {
            ...
            newSteps = plan.steps.map((s) => s.stepId === failedStepId
                ? {
                    ...s,
    ...
    async tryProceduralMemory(goal) {
        try {
            const proc = await this.memory.getBestProcedure(goal);
            if (proc && proc.successRate > 0.7)
                return proc;
        }
        catch {
    ...
    generateSteps(reasoning, toolNames, behavior) {
        const steps = [];
    ...
    estimateCost(steps) {
        ...
        for (const step of steps) {
            if (step.actionType === "tool_call")
                cost += COST_PER_TOOL_CALL;
            else
                cost += COST_PER_REASONING;
        }
