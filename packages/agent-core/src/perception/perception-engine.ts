...
export class PerceptionEngine {
    ...
    if (this.llmClient) {
      try {
        ...
        if (llmResult) {
          ...
          const result: PerceptionResult = PerceptionResultSchema.parse({
  ...
  private async llmPerceive(text: string): Promise<{
    intent: string;
  ...
  private extractText(message: AgentMessage): string {
    ...
    if (message.content && typeof message.content === "object" && "text" in (message.content as Record<string, unknown>)) {
  ...
  private classifyIntent(lowerText: string): string {
  ...
  private extractEntities(text: string): Array<{ name: string; type: string; value: unknown }> {
    const entities: Array<{ name: string; type: string; value: unknown }> = [];
  ...
  private classifyComplexity(
    text: string,
    entities: Array<{ name: string; type: string; value: unknown }>,
  ): "simple" | "medium" | "complex" {
    const wordCount = text.split(/\s+/).length;
    const hasCode = entities.some((e) => e.type === "code");

    if (wordCount > 200 || entities.length > 5 || hasCode) return "complex";
    if (wordCount > 50 || entities.length > 2) return "medium";
    return "simple";
  }

  /** Detect urgency from keywords. */
  private detectUrgency(lowerText: string): "low" | "medium" | "high" {
    ...
    if (highUrgency.some((kw) => lowerText.includes(kw))) return "high";
    if (lowUrgency.some((kw) => lowerText.includes(kw))) return "low";
    return "medium";
  }

  /** Detect domain from text and available tool names. */
  private detectDomain(lowerText: string, availableTools: string[]): string | undefined {
    // Check if any tool names appear in the text
    for (const tool of availableTools) {
      if (lowerText.includes(tool.toLowerCase())) return tool;
    }
    return undefined;
  }

  /** Extract caller style signals from message metadata. */
  private extractCallerStyleSignals(message: AgentMessage): Record<string, number> {
    const text = this.extractText(message);
