...
export class ExecutionEngine {
    ...
    for (const group of plan.parallelGroups) {
      ...
      if (this.consecutiveFailures >= this.circuitBreakerThreshold) {
        this.log.warn(
          ...
          "Circuit breaker tripped — aborting remaining steps",
        );
      ...
      switch (step.actionType) {
        case "tool_call": {
          ...
          const toolResult = await this.withTimeout(
            this.mcp.callTool(step.toolName, step.toolArguments ?? {}),
            this.stepTimeoutMs,
          );
  ...
  private async executeFallback(step: PlanStep): Promise<StepResult> {
    const start = Date.now();
    return {
      stepId: step.stepId,
      success: true,
  ...
  private async learnFromSuccess(plan: ExecutionPlan): Promise<void> {
