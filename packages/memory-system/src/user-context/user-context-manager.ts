...
export class UserContextManager {
  private readonly store: IUserContextStore;
  ...
  private async withCallerLock(callerId: string, fn: () => Promise<void>): Promise<void> {
  ...
  async getProfile(callerId: string): Promise<CallerProfile> {
    ...
    await this.withCallerLock(callerId, async () => {
      ...
      if (typeof interaction["preferredOutputFormat"] === "string") {
        profile.style.preferredOutputFormat =
          interaction["preferredOutputFormat"] as string;
      }
      ...
      if (typeof interaction["typicalSessionLength"] === "number") {
        profile.habits.typicalSessionLength =
          ...
          (interaction["typicalSessionLength"] as number) * alpha;
      }
  ...
  async updateSession(context: CallerContext): Promise<void> {
    await this.store.upsertSession(context);
  }
