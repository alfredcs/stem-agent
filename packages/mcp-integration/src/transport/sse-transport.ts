...
export class SSETransport implements IMCPTransport {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      this.readStream(res.body).catch((err: unknown) => {
        if (!this.connected) return; // expected on disconnect
  ...
  async send(method: string, params?: unknown): Promise<unknown> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        pending?.reject(
          new MCPTransportError("sse", `POST failed: HTTP ${res.status}`),
        );
      }
    } catch (err) {
      const pending = this.pending.get(id);
  ...
  private handleMessage(msg: Record<string, unknown>): void {
    if ("id" in msg && typeof msg["id"] === "number") {
      ...
      if ("error" in msg) {
        ...
        pending.reject(
          new MCPTransportError("sse", err.message ?? "Unknown RPC error"),
        );
      } else {
