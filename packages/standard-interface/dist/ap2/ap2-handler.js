...
import { AP2IntentMandateSchema, AP2PaymentMandateSchema, AP2PaymentReceiptSchema, ValidationError, AuthorizationError, } from "@stem-agent/shared";
/**
...
export class Ap2Handler {
    ...
    createRouter() {
        const router = Router();
        // -- Intent mandates ---------------------------------------------------
        router.post("/ap2/mandates/intent", async (req, res, next) => {
            try {
        ...
        router.get("/ap2/mandates/intent/:id", (req, res, next) => {
            try {
                const mandate = this.getActiveIntent(req.params.id);
                res.json(mandate);
            }
            catch (err) {
        ...
        // -- Payment mandates --------------------------------------------------
        router.post("/ap2/mandates/payment", async (req, res, next) => {
            try {
                const body = { id: uuidv4(), ...req.body };
        ...
        router.post("/ap2/mandates/payment/:id/approve", (req, res, next) => {
            try {
                ...
                if (payment.status !== "pending_approval") {
                    throw new ValidationError(`Cannot approve mandate in status "${payment.status}"`);
                }
                payment.status = "approved";
        ...
        router.post("/ap2/mandates/payment/:id/reject", (req, res, next) => {
            try {
                const payment = this.payments.get(req.params.id);
                if (!payment) {
                    res.status(404).json({ error: "Payment mandate not found" });
                ...
                if (payment.status !== "pending_approval") {
                    throw new ValidationError(`Cannot reject mandate in status "${payment.status}"`);
                }
        ...
        // -- Receipts ----------------------------------------------------------
        router.get("/ap2/receipts/:id", (req, res, next) => {
            try {
                const receipt = this.receipts.get(req.params.id);
                if (!receipt) {
        ...
        // -- Audit trail -------------------------------------------------------
        router.get("/ap2/audit/:intentId", (req, res, next) => {
            try {
                ...
                const linkedPayments = Array.from(this.payments.values()).filter((p) => p.intentMandateId === intent.id);
                const linkedReceipts = linkedPayments.flatMap((p) => {
                    const r = Array.from(this.receipts.values()).filter((receipt) => receipt.paymentMandateId === p.id);
                    return r;
                });
                res.json({
                    intent,
    ...
    executePayment(paymentMandateId, receipt) {
        const payment = this.payments.get(paymentMandateId);
        if (!payment)
            throw new ValidationError("Payment mandate not found");
    ...
    validateMerchant(intent, merchantId) {
        if (intent.allowedMerchants.length > 0 && !intent.allowedMerchants.includes(merchantId)) {
            throw new AuthorizationError(`Merchant "${merchantId}" not in allowed list`);
        }
    }
    validateCurrency(intent, currency) {
        if (currency !== intent.maxAmount.currency) {
            throw new ValidationError(`Currency "${currency}" does not match intent currency "${intent.maxAmount.currency}"`);
        }
    }
    validateAmount(intent, totalAmount) {
        const max = parseFloat(intent.maxAmount.amount);
        const total = parseFloat(totalAmount);
        if (total > max) {
            throw new ValidationError(`Total $${totalAmount} exceeds intent maximum $${intent.maxAmount.amount}`);
        }
    }
}
//# sourceMappingURL=ap2-handler.js.map