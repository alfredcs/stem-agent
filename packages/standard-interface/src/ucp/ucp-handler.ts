 ...
 */
function computeTotals(items: UcpCheckoutSession["lineItems"]): UcpCheckoutSession["totals"] {
  let subtotal = 0;
  let currency = "USD";
 ...
 */
function validateUcpHeaders(req: AuthenticatedRequest): string | null {
  if (!req.headers["ucp-agent"]) return "Missing required header: UCP-Agent";
  if (!req.headers["idempotency-key"]) return "Missing required header: Idempotency-Key";
  if (!req.headers["request-id"]) return "Missing required header: Request-Id";
  return null;
}
...
export class UcpHandler {
  private readonly sessions = new Map<string, UcpCheckoutSession>();
  private readonly idempotencyCache = new Map<string, UcpCheckoutSession>();
  ...
  createRouter(): Router {
    ...
    router.post("/ucp/checkout-sessions", async (req: AuthenticatedRequest, res) => {
    ...
    router.get("/ucp/checkout-sessions/:id", (req: AuthenticatedRequest, res) => {
    ...
    // Complete checkout
    router.post("/ucp/checkout-sessions/:id/complete", async (req: AuthenticatedRequest, res) => {
      const headerError = validateUcpHeaders(req);
      if (headerError) {
        res.status(400).json({ error: headerError });
        return;
