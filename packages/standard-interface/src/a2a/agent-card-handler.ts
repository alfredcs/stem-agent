...
import type { IStemAgent, AgentCard } from "@stem-agent/shared";

/**
 * Converts an AgentCard to A2A v0.3.0 wire format.
 */
function toA2AFormat(card: AgentCard): Record<string, unknown> {
  return {
    name: card.name,
    ...
    securitySchemes: Object.fromEntries(
      card.securitySchemes.map((s) => [s.name ?? s.scheme ?? "default", s]),
    ),
 ...
 */
export function agentCardRouter(agent: IStemAgent): Router {
  const router = Router();
