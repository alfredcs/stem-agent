...
import type { IStemAgent, AgentMessage, AgentResponse, IMemoryManager, IMCPManager } from "@stem-agent/shared";
 ...
 */
export function restRouter(deps: RestRouterDeps): Router {
   ...
   */
  router.post("/api/v1/tasks", async (req: AuthenticatedRequest, res, next) => {
    try {
   ...
   */
  router.post("/api/v1/chat", async (req: AuthenticatedRequest, res, next) => {
    try {
      const parsed = CreateTaskSchema.safeParse(req.body);
      if (!parsed.success) {
        throw new ValidationError("Invalid request body", {
   ...
   */
  router.get("/api/v1/chat/stream", async (req: AuthenticatedRequest, res, next) => {
    try {
      ...
      const agentMessage: AgentMessage = {
        ...
        role: "user",
        content: message,
        contentType: (req.query.contentType as string) ?? "text/plain",
        callerId: req.query.callerId as string | undefined,
