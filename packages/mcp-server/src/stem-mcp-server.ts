...
// ---------------------------------------------------------------------------
// StemMCPServer — exposes a running stem-agent as MCP tools
//
// This is the bridge between Claude Code and a live stem-agent instance.
// Claude Code connects via stdio transport; each tool maps to an IStemAgent
// or IMemoryManager method.
//
// Usage:
//   const server = new StemMCPServer(agent, memoryManager, mcpManager, persona);
//   await server.start();  // connects to stdio
// ---------------------------------------------------------------------------
...
export class StemMCPServer {
  ...
  private registerTools(): void {
    ...
    this.mcp.tool(
      "stem_process",
      "Send a message through the stem-agent pipeline (Perceive -> Reason -> Plan -> Execute). Returns the agent response.",
      {
    ...
    this.mcp.tool(
      "stem_list_skills",
      "List all skills registered in the stem-agent (both crystallized and plugin). Shows maturity, success rate, and triggers.",
      {},
      async () => {
    ...
    this.mcp.tool(
      "stem_agent_card",
      "Get the agent card describing this stem-agent's identity, capabilities, and available skills.",
      {},
      async () => {
        const card = this.agent.getAgentCard();
    ...
    this.mcp.tool(
      "stem_recall_memory",
      "Query the stem-agent's episodic memory. Returns relevant past interactions ranked by similarity.",
      {
    ...
    this.mcp.tool(
      "stem_health",
      "Check the health of the stem-agent and all connected MCP servers.",
      {},
      async () => {
    ...
    this.mcp.tool(
      "stem_get_persona",
      "Get the active domain persona configuration for this stem-agent instance.",
      {},
      async () => {
        if (!this.persona) {
          return {
            content: [{ type: "text" as const, text: "No domain persona configured. This is a generic stem-agent." }],
          };
        }
        return {
          content: [
