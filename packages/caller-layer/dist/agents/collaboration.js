...
export class DelegationPattern {
    ...
    aggregate;
    name = "delegation";
    log = createLogger("delegation-pattern");
    constructor(decompose, aggregate) {
...
export class ConsensusPattern {
    extractVote;
    name = "consensus";
    log = createLogger("consensus-pattern");
    threshold;
    weights;
    constructor(extractVote, opts) {
        this.extractVote = extractVote;
    ...
    async execute(task, agents) {
        ...
        this.log.info({ votes: Object.fromEntries(tally), reached, consensusValue }, "consensus vote complete");
        return {
            success: reached,
            results,
            metadata: {
...
export class PipelinePattern {
    stages;
    name = "pipeline";
    log = createLogger("pipeline-pattern");
    constructor(stages) {
        this.stages = stages;
    }
    async execute(task, _agents) {
