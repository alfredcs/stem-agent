...
export class CLI {
    ...
    lastTaskId = null;
    constructor(opts = {}) {
        const baseUrl = opts.baseUrl ?? process.env["STEM_AGENT_URL"] ?? "http://localhost:8000";
        this.callerId =
    ...
    showStatus() {
        const info = [
            ...
            `Agent:   ${this.client.baseUrl ?? "unknown"}`,
    ...
    showHistory() {
        ...
        const lines = this.history.map((h) => `[${h.role}] ${h.content}`);
        const text = lines.join("\n");
        this.write(text + "\n");
        return text;
    }
    async showMemory(callerId) {
